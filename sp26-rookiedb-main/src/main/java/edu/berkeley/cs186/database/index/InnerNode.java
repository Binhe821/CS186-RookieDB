...
class InnerNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        // TODO(proj2): implement
        sync();
        int order = metadata.getOrder(), index = 0;
        int flag = 0;
        for (int i = 0; i < keys.size(); ++i) {
            if (key.compareTo(keys.get(i)) < 0) {
                index = i;
        ...
        Optional<Pair<DataBox, Long>> toInsert = getChild(index).put(key, rid);
        sync();
        Pair<DataBox, Long> pair = null;
        if (toInsert.isPresent()) {
            DataBox splitNext = toInsert.get().getFirst();
            //keys.add(splitNext); //splitkey uploaded from next page
            Long pageNum = toInsert.get().getSecond();

            index = 0;
            for (int i = 0; i < keys.size(); ++i) {
                if (splitNext.compareTo(keys.get(i))<0) {
                    index = i;
                    break;
                }
                else if (i == keys.size() - 1) index = i + 1;
            }
            keys.add(index, splitNext);
            children.add(index + 1, pageNum);
            if (keys.size() > 2 * order) {
                flag = 1;
                ArrayList<DataBox> newKeys = new ArrayList<>(keys.subList(order + 1, 2 * order + 1));
                ArrayList<Long> newChild = new ArrayList<>(children.subList(order + 1, 2 * order + 2));
                DataBox toPush = keys.get(order);
                keys.subList(order, 2 * order + 1).clear();
                children.subList(order + 1, 2 * order + 2).clear();
                InnerNode innerNode =  new InnerNode(metadata,bufferManager,newKeys,newChild,treeContext);
                pair = new Pair<>(toPush, innerNode.page.getPageNum());
            }
        }
        sync();
        return flag == 0 ? Optional.empty() : Optional.of(pair);
    }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        while(data.hasNext()) {
            ...
            flag = 0;
            Optional<Pair<DataBox, Long>> toInsert = getChild(children.size() - 1).bulkLoad(data, fillFactor);
            pair = null;
            if (toInsert.isPresent()) {
                DataBox splitNext = toInsert.get().getFirst(), splitKey = null;
                //keys.add(splitNext); //splitkey uploaded from next page
                Long pageNum = toInsert.get().getSecond();
                keys.add(splitNext);
                ...
                if (keys.size() > 2 * order) {
                    flag = 1;
                    ArrayList<DataBox> newKeys = new ArrayList<>(keys.subList(order, 2 * order+1));
                    ArrayList<Long> newChild = new ArrayList<>(children.subList(order + 1, 2 * order + 2));
                    keys.subList(order, 2 * order + 1).clear();
                    children.subList(order + 1, 2 * order + 2);
                    InnerNode innerNode = new InnerNode(metadata, bufferManager, newKeys, newChild, treeContext);
                    splitKey = newKeys.get(0);
                    newKeys.remove(0);
                    pair = new Pair<>(splitKey, innerNode.page.getPageNum());
                }
                sync();
                return flag == 0 ? Optional.empty() : Optional.of(pair);
            }
