...
public class SortOperator extends QueryOperator {
    ...
    public Run sortRun(Iterator<Record> records) {
        ...
        while(records.hasNext()) {
            record.add(records.next());
        }
        record.sort(new Comparator<Record>() {
            @Override
            public int compare(Record leftRecord, Record rightRecord) {
                DataBox leftRecordValue = leftRecord.getValue(sortColumnIndex);
                DataBox rightRecordValue = rightRecord.getValue(sortColumnIndex);
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        // TODO(proj3_part1): implement
        PriorityQueue<Pair<Record, Integer>> record = new PriorityQueue<>(new RecordPairComparator());
        Run run = new Run(transaction, getSchema());
        ArrayList<Iterator<Record>> iter = new ArrayList<>();
        for (Run r: runs) {
            iter.add(r.iterator());
        }
        for (int i = 0; i < runs.size(); i++) {
            record.add(new Pair<Record, Integer>(iter.get(i).next(), i));
        }
        while (!record.isEmpty()) {
            Pair<Record, Integer> first = record.poll();
            run.add(first.getFirst());
            int indexAdd = first.getSecond();
            if (iter.get(indexAdd).hasNext())
                record.add(new Pair<Record, Integer>(iter.get(indexAdd).next(),indexAdd));
        }
        return run;
    }
