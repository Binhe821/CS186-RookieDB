...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        // TODO(proj2): implement
        sync();
        int flag = 0, order = metadata.getOrder();
        int index = 0;
        Pair<DataBox, Long> pair = null;
        for (int i = 0; i < keys.size(); ++i) {
            if (key.compareTo(keys.get(i)) == 0) throw new BPlusTreeException("Duplicate Element!");
        ...
        if (keys.size() > 2 * order) {
            ...
            ArrayList<RecordId> newRids = new ArrayList<>(rids.subList(order, 2 * order + 1));
            keys.subList(order, 2 * order + 1).clear();
            rids.subList(order, 2 * order + 1).clear();
            Optional<Long> sib = rightSibling;
            DataBox splitKey = newKeys.get(0);
            LeafNode leafNode = new LeafNode(metadata, bufferManager, newKeys, newRids, sib, treeContext);
            pair = new Pair<>(splitKey, leafNode.page.getPageNum());
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        // TODO(proj2): implement
        sync();
        int order = metadata.getOrder(),fullNum = (int) Math.ceil(2 * order * fillFactor);
        while(data.hasNext() && keys.size() <= fullNum) {
            Pair<DataBox, RecordId> toInsert = data.next();
            keys.add(toInsert.getFirst());
            rids.add(toInsert.getSecond());
        }
        int flag = 0;
        Pair<DataBox, Long> pair = null;
        if (keys.size() > fullNum) {
            flag = 1;
            ArrayList<DataBox> newKeys = new ArrayList<>(keys.subList(fullNum, keys.size()));
            ArrayList<RecordId> newRids = new ArrayList<>(rids.subList(fullNum, rids.size()));
            keys.subList(fullNum, keys.size()).clear();
            rids.subList(fullNum, rids.size()).clear();
            Optional<Long> sib = rightSibling;
            LeafNode leafNode = new LeafNode(metadata, bufferManager, newKeys, newRids, sib, treeContext);
            pair = new Pair<>(newKeys.get(0), leafNode.page.getPageNum());
            rightSibling = Optional.of(leafNode.getPage().getPageNum());
        }
        sync();
        return flag == 0 ? Optional.empty() : Optional.of(pair);
