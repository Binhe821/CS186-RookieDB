...
public class BPlusTree {
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        while(data.hasNext())
        {
            ...
            if (pair.isPresent()) {
                ...
                keys.add(pair.get().getFirst());
                ArrayList<Long> children = new ArrayList<>();
                children.add(this.metadata.getRootPageNum());
                children.add(pair.get().getSecond());
                BPlusNode newRoot = new InnerNode(metadata, bufferManager, keys, children, lockContext);
                updateRoot(newRoot);
            }
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        public BPlusTreeIterator(DataBox mini)
        {
            current = root.get(mini);
            iter = current.scanGreaterEqual(mini);
        }
        @Override
        public boolean hasNext() {
            // TODO(proj2): implement
            if (iter.hasNext()) return true;
            while (current.getRightSibling().isPresent()) {
