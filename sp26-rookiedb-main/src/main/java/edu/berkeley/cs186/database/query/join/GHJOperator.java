...
public class GHJOperator extends JoinOperator {
    ...
    private void partition(Partition[] partitions, Iterable<Record> records, boolean left, int pass) {
        ...
        for (Record r: records) {
            DataBox columnValue;
            if (left)
            columnValue = r.getValue(getLeftColumnIndex());
            else columnValue = r.getValue(getRightColumnIndex());
            int hash = HashFunc.hashDataBox(columnValue, pass);
            int partitionNum = hash % partitions.length;
            if (partitionNum < 0) {
                partitionNum += partitions.length;
            }
            partitions[partitionNum].add(r);
        }
        return;
    }
    ...
    private void buildAndProbe(Partition leftPartition, Partition rightPartition) {
        ...
        for (Record build: buildRecords) {
            DataBox joinValue = build.getValue(buildColumnIndex);
            if (!hashTable.containsKey(joinValue)) {
                hashTable.put(joinValue, new ArrayList<>());
            }
            hashTable.get(joinValue).add(build);
        }

        for (Record probe: probeRecords) {
            DataBox joinValue = probe.getValue(probeColumnIndex);
            if (!hashTable.containsKey(joinValue)) continue;
            for (Record build: hashTable.get(joinValue)) {
                Record joined;
                if (!probeFirst) {
                    joined = build.concat(probe);
                }
                else joined = probe.concat(build);
                this.joinedRecords.add(joined);
            }
        }
    }
    ...
    private void run(Iterable<Record> leftRecords, Iterable<Record> rightRecords, int pass) {
        ...
        for (int i = 0; i < leftPartitions.length; i++) {
            ...
            Partition leftP = leftPartitions[i];
            Partition rightP = rightPartitions[i];
            if (leftP.getNumPages() > this.numBuffers - 2 && rightP.getNumPages() > this.numBuffers - 2) {
                run(leftP, rightP, pass + 1);
            } else buildAndProbe(leftP, rightP);
        }
    }
